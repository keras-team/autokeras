...
from autokeras.legacy.nn.model_trainer import ModelTrainerBase
...
class ModelTrainer(ModelTrainerBase):
    ...
    def __init__(self, model, path, **kwargs):
        ...
        if self.loss_function.__name__ == 'classification_loss':
            self.keras_metric = tf.keras.metrics.categorical_accuracy
        elif self.loss_function.__name__ == 'regression_loss':
            self.keras_metric = tf.keras.metrics.mean_squared_error
        elif self.loss_function.__name__ == 'binary_classification_loss':
            self.keras_metric = tf.keras.metrics.binary_accuracy
    ...
    def train_model(self,
        ...
        self.scheduler = tf.keras.callbacks.LearningRateScheduler(lr_schedule)
        self.reducer = tf.keras.callbacks.ReduceLROnPlateau(factor=np.sqrt(0.1), cooldown=0, patience=5, min_lr=0.5e-6)
