...
from autokeras.legacy.nn.layers import StubAdd, StubConcatenate, StubConv1d, StubConv2d, StubConv3d, \
...
class KerasModel:
    ...
    def __init__(self, graph):
        ...
        input_tensor = tf.keras.layers.Input(shape=graph.node_list[input_id].shape)
        ...
        self.model = tf.keras.models.Model(inputs=input_tensor, outputs=output_tensor)
...
def to_real_keras_layer(stub_layer):
    if isinstance(stub_layer, StubConv1d):
        return tf.keras.layers.Conv1D(stub_layer.filters,
        ...
        return tf.keras.layers.Conv2D(stub_layer.filters,
                             stub_layer.kernel_size,
                             strides=stub_layer.stride,
                             input_shape=stub_layer.input.shape,
                             padding='same')  # padding
        ...
        return tf.keras.layers.Conv3D(stub_layer.filters,
                             stub_layer.kernel_size,
                             strides=stub_layer.stride,
                             input_shape=stub_layer.input.shape,
                             padding='same')  # padding
    ...
    # TODO: Spatial Dropout
        ...
        return tf.keras.layers.Dropout(stub_layer.rate)
    ...
    #     return layers.SpatialDropout3D(stub_layer.rate)
        ...
        return tf.keras.layers.AveragePooling1D(stub_layer.kernel_size, strides=stub_layer.stride)
    elif isinstance(stub_layer, StubAvgPooling2d):
        return tf.keras.layers.AveragePooling2D(stub_layer.kernel_size, strides=stub_layer.stride)
    elif isinstance(stub_layer, StubAvgPooling3d):
        return tf.keras.layers.AveragePooling3D(stub_layer.kernel_size, strides=stub_layer.stride)
        ...
        return tf.keras.layers.MaxPooling1D(stub_layer.kernel_size, strides=stub_layer.stride)
    elif isinstance(stub_layer, StubPooling2d):
        return tf.keras.layers.MaxPooling2D(stub_layer.kernel_size, strides=stub_layer.stride)
    elif isinstance(stub_layer, StubPooling3d):
        return tf.keras.layers.MaxPooling3D(stub_layer.kernel_size, strides=stub_layer.stride)

    elif isinstance(stub_layer, (StubBatchNormalization1d, StubBatchNormalization2d, StubBatchNormalization3d)):
        return tf.keras.layers.BatchNormalization(input_shape=stub_layer.input.shape)
        ...
        return tf.keras.layers.Dense(stub_layer.units, input_shape=(stub_layer.input_units,))
