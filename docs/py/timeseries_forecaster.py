...
data_x = data_train[['CO(GT)', 'PT08.S1(CO)', 'NMHC(GT)', 'C6H6(GT)',
                     'PT08.S2(NMHC)', 'NOx(GT)', 'PT08.S3(NOx)', 'NO2(GT)',
                     'PT08.S4(NO2)', 'PT08.S5(O3)', 'T', 'RH']].astype('float64')

data_x_val = validation_data[['CO(GT)', 'PT08.S1(CO)', 'NMHC(GT)', 'C6H6(GT)',
                              'PT08.S2(NMHC)', 'NOx(GT)', 'PT08.S3(NOx)', 'NO2(GT)',
                              'PT08.S4(NO2)', 'PT08.S5(O3)', 'T', 'RH']].astype(
                                  'float64')
...
clf.fit(x=data_x, y=data_y, validation_data=(data_x_val, data_y_val), batch_size=30,
        epochs=10)

predictions = clf.predict(data_x_val)
