...
def test_hill_climbing_classifier_generator():
    ...
    constant.MAX_ITER_NUM = 3
    constant.MAX_MODEL_NUM = 1
    generator = HillClimbingClassifierGenerator(3, (28, 28, 1),x_train=x_train,y_train=y_train,x_test=x_test,y_test=y_test,verbose=False)
    model = None
    generate_model = generator.generate()
    times = 1
    while generate_model !=  None and times <= constant.MAX_MODEL_NUM:
