...
def generate_structured_data(num_instances=500, dtype="np"):
    ...
    col_num_to_cat = np.random.randint(20, 41, size=num_instances).reshape(
        num_instances, 1
    )
    col_float = 100 * np.random.random(num_instances,).reshape(num_instances, 1)
    col_int = np.random.randint(2000, 4000, size=num_instances).reshape(
        num_instances, 1
    )
    col_morethan_32 = np.random.choice(career_size, num_instances).reshape(
        num_instances, 1
    )
    col1_morethan_100 = np.random.choice(career_states, num_instances).reshape(
        num_instances, 1
    )
    col2_morethan_100 = np.random.choice(career_years, num_instances).reshape(
        num_instances, 1
    )
    col3_morethan_100 = np.random.choice(career_color, num_instances).reshape(
        num_instances, 1
    )
    data = np.concatenate(
        (
...
def fit_predict_with_graph(inputs, outputs, x, y):
    model = ak.graph.HyperBuiltGraphHyperModel(inputs, outputs).build(
        kerastuner.HyperParameters()
    )
    model.fit(x, y, epochs=1, batch_size=100, verbose=False, validation_split=0.2)
